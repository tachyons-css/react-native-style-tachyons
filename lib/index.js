...
var NativeTachyons = {
    ...
    build: function build() {
        ...
        _lodash2.default.forEach(REM_SCALED, function (subSheet) {
            ...
            /* sizes for export */
            _lodash2.default.forEach(subSheet, function (rule, tachyonsKey) {
        ...
        var allColors = _lodash2.default.transform(options.colors.palette, function (result, val, key) {
            ...
            if (options.colors.lighten !== false) {
                result["light-" + key] = new _color2.default(val).lighten(options.colors.lighten).hexString();
            }
            if (options.colors.darken !== false) {
                result["dark-" + key] = new _color2.default(val).darken(options.colors.darken).hexString();
            }

            /* alpha variants */
            for (var i = 10; i < 100; i += 10) {
                var name = key + "-" + i;
                var rgbString = new _color2.default(val).alpha(i / 100).rgbString();
        ...
        /* font-families */
        _lodash2.default.forEach(options.fonts, function (val, key) {
